...
json_graph_t = "bds/base_ExtraitBilletterie-SansSalles.json"
json_graphe = "/home/franckeinstine/PythonCode/cpy_real_code/graphsFiles/influencers.json"
...
class methodes_complementaires():
	"""docstring for methodes_complementaires"""
	...
	#La methode permet la lecture d'un fichier JSON
	def readJsonFile(self, filename ):   #il prend en parametre le nome de fichier à lire ex. path/de/fichier.json
		d = json.load(open(filename))   #par ici nous ouvrons le ficher et on crée un dictionnaire de contennus du fichier
		return json_graph.node_link_graph(d) #et on retourne le contenu du fichier, ici particulairement le liens de noeud du graphe

	def saveGraph(self, G, fname):
		json.dump(dict(nodes=[[n, G.node[n]] for n in G.nodes()],
						edges=[[u, v, G.edge[u][v]] for u,v in G.edges()]),
					open(fname, 'w'), indent=2)

	def all_blocks(self, graph):
		blocks = nx.get_node_attributes(graph, "block")
		Id_influ = nx.get_node_attributes(graph, "id")
		#infu = 'influence'
		#blocks = influencers.values()
		blockDict = { }
		DicInf_block = { }
		for u in graph.nodes():
			inflluencers = []
			Id_inflluencers = []
			for clef in blocks.values():
				inflluencers.append(str(clef))
			for id_clef in blocks.keys():
				Id_inflluencers.append(str(id_clef))
		influeDict = dict(zip(Id_inflluencers, inflluencers))
		return blockDict
...
class weightFunctions:
	...
	def all_influencees(self, graph):
		influencers = nx.get_node_attributes(graph, "influence")
		Id_influ = nx.get_node_attributes(graph, "id")
		infu = 'influence'
		#blocks = influencers.values()
		influeDict = { }
		...
		for u in graph.nodes():
			inflluencers = []
			Id_inflluencers = []
			for clef in influencers.values():
				inflluencers.append(str(clef))
			for id_clef in influencers.keys():
				Id_inflluencers.append(str(id_clef))
		influeDict = dict(zip(Id_inflluencers, inflluencers))
		return influeDict
    ...
    def sumOverNeighborsInWfunction( self, graph, w, u ): 
		graph_json = methodes_complementaires().read_json_file(json_graphe)
		#block = nx.get_node_attributes(graph_json, "block") 
		dicBlocks = methodes_complementaires().all_blocks(graph_json)

		#for v in graph.nodes(): 
		valOk = 12
		blocks_equals = {}
		for n in graph_json.nodes():
			for clef, valeur in dicBlocks.iteritems():
				if str(valOk) in valeur:
					blocks_equals[str(clef)] = str(valeur)

			if u in blocks_equals:
